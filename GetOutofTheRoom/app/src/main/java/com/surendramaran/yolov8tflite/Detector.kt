...
class Detector(
    ...
    fun detect(frame: Bitmap) {
        ...
        // Process image (normalize, cast, etc.)
        ...
        // Handle case of no bounding boxes
        if (bestBoxes == null) {
            detectorListener.onEmptyDetect()
            return
        }

        // Return the bounding boxes and inference time to the listener
    ...
    private fun bestBox(array: FloatArray): List<BoundingBox>? {
        ...
        for (c in 0 until numElements) {
            val cx = array[c] // x center
            val cy = array[c + numElements] // y center
            ...
            if (conf > CONFIDENCE_THRESHOLD) {
                ...
                // Ignore boxes that are out of bounds
                if (x1 < 0F || x1 > 1F || y1 < 0F || y1 > 1F || x2 < 0F || x2 > 1F || y2 < 0F || y2 > 1F) {
                    continue
                }

                // Assuming only one class "Door"
                boundingBoxes.add(
                    BoundingBox(
                        x1 = x1, y1 = y1, x2 = x2, y2 = y2,
                        cx = cx, cy = cy, w = w, h = h,
                        cnf = conf, cls = 0, clsName = "Door"
                    )
                )
            }
        }
